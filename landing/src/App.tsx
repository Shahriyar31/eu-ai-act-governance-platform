...
const LIVE = "https://eu-ai-governance.salmonocean-15ddaf55.germanywestcentral.azurecontainerapps.io"
        ...
        {w >= 900 && (
          <div style={{ position: "relative", height: 360, width: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
            {cards.map((item, idx) => {
              ...
              else if (rel < 0) { tx = rel * 480; ty = rel * -18; tz = rel * 80; sc = 1 + rel * 0.08; rx = rel * 6; ry = rel * -3; op = Math.max(0, 1 + rel * 0.4) }
              else { tx = rel * 12; ty = rel * 16; tz = rel * -90; sc = 1 - rel * 0.05; op = Math.max(0, 1 - rel * 0.28); rx = rel * -5 }
              const zi = Math.abs(rel) < 0.05 ? 100 : rel < 0 ? 40 + Math.floor(rel * 5) : 90 - Math.floor(rel * 10)
              const isActive = Math.abs(rel) < 0.05
              return (
                <div key={idx} style={{ position: "absolute", width: 380, height: 295, transformStyle: "preserve-3d", transform: `translateX(${tx}px) translateY(${ty}px) translateZ(${tz}px) scale(${sc}) rotateX(${rx}deg) rotateY(${ry}deg)`, opacity: op, zIndex: zi, transition: "transform 0.05s linear, opacity 0.05s linear" }}>
                  <UnifiedCard color={item.color} isDeep beam={isActive}>
                    <div>
                      ...
                      <div style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontWeight: 800, fontSize: "1.1rem", color: "var(--text-primary)", marginBottom: 10 }}>{item.title}</div>
    ...
    <AnimatePresence>
      {open && (
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
          ...
          {[["#features", "Capabilities"], ["#demo", "Audit Ledger"], ["#stack", "Architecture"], ["#pricing", "Pricing"]].map(([h, l]) => (
